from Planner.web.main import app
from flask import request
from Planner.web.Adapters import check_args, generate_answer, \
...
def update():
    ...
    name, description, priority, progress = None, None, None, None
    if check_args(request.args, 'name'):
        name = request.args['name']
    if check_args(request.args, 'description'):
        description = request.args['description']
    if check_args(request.args, 'progress'):
    ...
    if name is None and description is None and priority is None and progress is None:
        return generate_answer(False, error_code=2)
    query(get_update_sql(task_id, name, description, priority, progress))
    return generate_answer(True, {})
